...
const EditorAside = ({ clients, id, socket, username }) => {
  ...
  return (
    <div className="flex flex-row md:flex-col justify-between bg-indigo-900 w-full h-16 md:w-60 md:h-screen transition-all duration-100 md:px-4 md:py-2 px-4 py-4 items-center relative z-10">
      <div className="flex flex-row space-x-6 md:space-x-0 md:flex-col md:space-y-6 items-start">
        ...
        <div
          className="h-6 w-1 md:w-full md:h-2 rounded-sm bg-slate-300 mx-auto "
          id="divider"
        ></div>
        <div className="flex flex-col space-y-4 ">
          ...
          <div
            id="user-connected"
            className="hidden md:flex gap-6 flex-wrap items-start  flex-grow h-full rounded-lg"
          >
      ...
      <div className="flex flex-row space-x-4 md:flex-col md:space-y-6 md:space-x-0 md:w-full relative">
        ...
        >
          {" "}
          <i className="fa-regular fa-clipboard"></i> Copy Room ID
        </button>
        <button
          className="bg-red-600 font-josefin text-sm md:text-md rounded-md shadow-md px-2 md:py-2 "
          onClick={leaveRoom}
        >
          {" "}
          Leave Room
